defmodule Mango.Web.Admin.SessionController do
  ...
  def send_link(conn, %{"session" => %{"email" => email}}) do
   ...
   defp verify_token(token) do
     Phoenix.Token.verify(Mango.Web.Endpoint, "user", token, max_age: @max_age)
   end

  defp generate_login_link(conn, user) do
    token = Phoenix.Token.sign(Mango.Web.Endpoint, "user", user.id)
